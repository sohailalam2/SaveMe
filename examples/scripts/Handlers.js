...
// Register Handlers
saveMe.registerErrorHandler("ERROR", errorHandler)
    .registerSuccessHandler("SAVE_SUCCESS", saveSuccessHandler)
    .registerSuccessHandler("REMOVE_SUCCESS", removeSuccessHandler);

function errorHandler(type, msg) {
    alert("TYPE:\n" + type + "\n\nMESSAGE:\n" + msg);
}

function saveSuccessHandler(type, msg) {
    if (type === SaveMe._SUCCESS_SAVE_)
        alert("Yeehh...\n\n" + msg);
}

function removeSuccessHandler(type, msg) {
    if (type === SaveMe._SUCCESS_REMOVE_)
        alert("OOO...\n\n" + msg);
}
